    ...
    , m_fontSize(m_dconfig->value("fontSize", 105).toUInt())
{
    ...
    m_configThread.reset(new QThread);
    m_config.reset(new org_deepin_treeland(m_configThread.get(),
                                           "org.deepin.treeland",
                                           "org.deepin.treeland",
                                           QString()));
}

TreelandConfig::~TreelandConfig() = default;
...
{
    m_config->setMaxWorkspace(newMaxWorkspace);
...
{
    m_config->setNumWorkspace(newNumWorkspace);
...
{
    m_config->setCurrentWorkspace(newCurrentWorkspace);
...
{
    return m_config->forceSoftwareCursor();
    // m_forceSoftwareCursor = m_dconfig->value("forceSoftwareCursor", false).toBool();
...
{
    m_config->setForceSoftwareCursor(enable);
...
{
    m_config->setCursorThemeName(theme);
...
{
    m_config->setCursorSize(size.width());
...
{
    m_config->setWindowThemeType(type);
...
{
    return m_config->windowThemeType();
    // m_windowThemeType = m_dconfig->value("windowThemeType", 0).toUInt();
...
{
    m_config->setWindowTitlebarHeight(height);
...
{
    return m_config->windowTitlebarHeight();
    // m_windowTitlebarHeight = m_dconfig->value("windowTitlebarHeight", 30).toUInt();
...
{
    m_config->setMonoFont(monoFontName);
